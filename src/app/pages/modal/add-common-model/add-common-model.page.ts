...
import { ModalController, Platform, NavParams, LoadingController, NavController } from '@ionic/angular';
...
export class AddCommonModelPage implements OnInit, OnDestroy {
  ...
  private getPropertySubscribe: Subscription;
  ...
  ngOnInit() {
    ...
    this.href = this.router.url;
        console.log(this.router.url);// today select
    ...
    this.getid = this.navParams.get('modalForm_item');
    console.log("commercial id.......", this.getid);


    if(this.get_identifier == 'jointeam')
    {
      this.heder_title = 'Join My Team';
      this.onEditField = 'POST';
      this.api_url = 'api/join-our-team';

    }else if(this.get_identifier == 'signIn'){
      this.heder_title = 'Sign In';
      this.api_url = 'api/login';

    }else if(this.get_identifier == 'signUp'){
      this.heder_title = 'Sign Up';
      this.onEditField = 'POST';
      this.api_url = 'api/register';

    }else if(this.get_identifier == 'property'){
      this.heder_title = 'Property';
      this.api_url = 'api/make-request';
      this.getCategory();
  ...
  init(_getid){
    this.model = {
      property_category : _getid,
    }
    console.log("_getid",_getid);
    
    if(_getid != 6)
    {
      this.getProperty(_getid);
    }
  ...
  // =========== Commercial Submit start ===========
  onCommercialSubmit(form:NgForm){
    this.form_submit_text = 'Submitting';

    // get form value
    let fd = new FormData();
    fd.append('property_category', this.getid);
    for (let val in form.value) {
      if(form.value[val] == undefined){
        form.value[val] = '';
      }
...
onChangeProperty(_item){
  console.log("Change Property....",_item);
  // for (let i = 0; i < this.propertyList.length; i++) {
  //   if(this.propertyList[i].id == _item){
  //     this.category = this.propertyList[i].category;
  //     console.log("Property Category....", this.category);
  //   }
  // }
}
onChangeCategory(_item){
  console.log("Change Category....", _item);
  this.getProperty(_item);
  
}

// Property data fetch start
getCategory(){
  this.plt.ready().then(() => {
    this.selectLoading = true;
    this.getCategorySubscribe = this.commonUtils.getlistCommon('api/property-category').subscribe(
      resData => {
        console.log("getCategory Data...",resData);
        this.selectLoading = false;
...
// Property data fetch end

// Property data fetch start
getProperty(_item){
  this.plt.ready().then(() => {
    this.selectLoading = true;
    this.getPropertySubscribe = this.commonUtils.getlistCommon('api/get-property/'+_item).subscribe(
      resData => {
        console.log("getProperty Data...",resData);
        this.selectLoading = false;
...
getFooter(){
  ...
  this.getFooterSubscribe = this.http.get('api/footer').subscribe(
    (res:any) => {
    this.selectLoadingDepend = false;
    this.logo =res.return_data;
  ...
  ngOnDestroy(){
    ...
    if(this.getPropertySubscribe !== undefined){
      this.getPropertySubscribe.unsubscribe();
    }
