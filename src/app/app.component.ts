...
export class AppComponent implements OnInit {
  ...
  private footerDataSubscribe : Subscription;
  menuPages = [];
  menuPagesList;
  menuPages2 = [];
  activeMenuHilight;
  ...
  footerData;
  footerDataGet(){
    this.footerDataSubscribe = this.http.get('api/footer').subscribe(
      (res:any) => {
        console.log("view data  res -------------------footer ss ->", res.return_data);
        if(res.return_status > 0){
          this.footerData = res.return_data;

          // pageTitle
          this._document.getElementById('pageTitle').innerHTML = res.return_data.all_data.name;

          // fabicon set
          this._document.getElementById('appFavicon').setAttribute('href', this.file_url+'/'+res.return_data.all_data.path+'/'+res.return_data.all_data.fab_icon);
        }
      },
      errRes => {
